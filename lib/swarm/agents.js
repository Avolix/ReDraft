 ...
 * @param {number} [opts.timeoutSeconds] - Per-agent timeout override
 * @returns {Promise<string>} Raw LLM response text
 */
export async function callAgent({ systemPrompt, promptText, refineFn, signal, modelOverride, timeoutSeconds }) {
    return refineFn(promptText, systemPrompt, { signal, model: modelOverride || undefined, timeout: timeoutSeconds || undefined });
}
