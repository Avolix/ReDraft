...
async function executePipeline({ strategyConfig, messageText, contextBlock, allBuiltInRules, settings, refineFn, signal, onProgress }) {
...
async function executeCouncil({ strategyConfig, messageText, contextBlock, fullRulesText, settings, refineFn, signal, onProgress }) {
    ...
    for (const batch of batches) {
        ...
        const batchResults = await Promise.all(batch.map(async ({ emphasis, idx }) => {
            ...
            return { agentName, durationMs, rawResponse };
        }));

        agentLog.push(...batchResults);
        candidates.push(...batchResults.map(r => r.rawResponse));
    }
    ...
    const judgeSystemPrompt = buildCouncilJudgePrompt(councilSize, judgeMode);
    ...
    agentLog.push({ agentName: 'Judge', durationMs: judgeDuration, rawResponse: judgeResponse });

    onProgress?.({
        phase: 'Council',
        current: councilSize + 1,
        total: councilSize + 1,
        agentName: 'Judge',
        status: 'done',
    });
...
async function executeReviewRefine({ messageText, contextBlock, fullRulesText, refineFn, signal, onProgress }) {
    const agentLog = [];
