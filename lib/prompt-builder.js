...
export const USER_POV_INSTRUCTIONS = {
    '1st': 'PERSPECTIVE RULES (MANDATORY): "I/me/my" = the user\'s character ONLY. All other characters (including the AI character) = "he/she/they" + names in narration. Do not swap, mix, or shift pronoun assignments. If any pronoun violates this map, fix it.',
    '2nd': 'PERSPECTIVE RULES (MANDATORY): "you/your" = the user\'s character in all narration and descriptions. All other characters = "he/she/they" + names. No "I" in narrative voice. If any pronoun violates this map, fix it.',
    '3rd': 'PERSPECTIVE RULES (MANDATORY): ALL characters (including the user\'s character) use "he/she/they" + names. No "I" in narration. No "you" in narration. If any pronoun violates this map, fix it.',
};
...
1. First, output a changelog inside [CHANGELOG]...[/CHANGELOG] tags listing each change you made and which rule motivated it. One line per change made. ONLY list changes you actually made. Do not list rules that passed, items you considered but left unchanged, or deliberation. If you checked a rule and changed nothing, do not mention it.
...
/**
 * Extract known XML tag blocks from reasoning/CoT text.
 * Looks for Simulacra-style tags: <society>, <power_dynamic>, <conviction>, <language_output>.
 * @param {string} reasoning - The raw reasoning/CoT text
 * @returns {string|null} Extracted tag content joined by newlines, or null if none found
 */
export function extractReasoningTags(reasoning) {
    if (!reasoning) return null;
    const tagNames = ['society', 'power_dynamic', 'conviction', 'language_output'];
    const found = [];
    for (const tag of tagNames) {
        const regex = new RegExp(`<${tag}>[\\s\\S]*?</${tag}>`, 'gi');
        const matches = reasoning.match(regex);
        if (matches) {
            found.push(...matches);
        }
    }
...
/**
 * Truncate reasoning text, keeping the tail (most recent context).
 * @param {string} reasoning - The raw reasoning/CoT text
 * @param {number} maxChars - Maximum characters to keep
 * @returns {string} Truncated text
 */
export function truncateReasoning(reasoning, maxChars) {
    if (!reasoning || reasoning.length <= maxChars) return reasoning || '';
    return '...' + reasoning.slice(-maxChars);
}
...
/**
 * Build the context block (character, chat history, PoV, reasoning) shared by
 * single-pass refinement and swarm multi-agent refinement.
 *
 ...
 * @returns {string} The formatted context block (empty string if no context available)
 */
export function buildRefineContextBlock(settings, context, chatArray, messageIndex, strippedMessage, { reasoning } = {}) {
    const contextParts = [];
    ...
    if (reasoning && settings.reasoningContext) {
        const mode = settings.reasoningContextMode || 'tags';
        let reasoningBlock = null;
        if (mode === 'tags') {
            reasoningBlock = extractReasoningTags(reasoning);
        }
        if (!reasoningBlock && (mode === 'raw' || (mode === 'tags' && settings.reasoningContextRawFallback))) {
            const maxChars = Math.min(4000, Math.max(200, settings.reasoningContextChars ?? 1000));
            reasoningBlock = truncateReasoning(reasoning, maxChars);
        }
        if (reasoningBlock) {
            contextParts.push(`Scene settings (from model reasoning):\n${reasoningBlock}`);
        }
    }
 ...
 * @param {number} messageIndex - Index of the AI message being refined
 * @param {string} strippedMessage - Message text with protected blocks replaced
 * @param {{ rulesText: string, systemPrompt: string, reasoning?: string }} opts - Pre-resolved rules, system prompt, and optional CoT reasoning
 * @returns {{ systemPrompt: string, promptText: string }}
 */
export function buildAiRefinePrompt(settings, context, chatArray, messageIndex, strippedMessage, { rulesText, systemPrompt, reasoning }) {
    const contextBlock = buildRefineContextBlock(settings, context, chatArray, messageIndex, strippedMessage, { reasoning });
    const contextPrefix = contextBlock ? contextBlock + '\n\n' : '';

    const promptText = `${contextPrefix}Apply the following refinement rules to the message below. Any [PROTECTED_N] placeholders are protected regions \u2014 output them exactly as-is.
...
export function buildUserEnhancePrompt(settings, context, chatArray, messageIndex, strippedMessage, { rulesText, personaDesc, systemPrompt }) {
    ...
    const povKey = resolvePov(settings.userPov ?? settings.pov, strippedMessage);
    if (povKey !== 'auto' && USER_POV_INSTRUCTIONS[povKey]) {
        contextParts.push(`Point of view: ${USER_POV_INSTRUCTIONS[povKey]}`);
    }
