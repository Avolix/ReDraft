...
export function categorizeRefinementError(errorMessage) {
    ...
    if (msg.includes('timed out')) {
        return {
            toastMessage: 'Refinement timed out \u2014 try increasing the timeout in ReDraft\'s Advanced settings, use a faster model, or check your API provider\'s status page.',
            timeOut: 8000,
        };
    }
    ...
    if (msg.includes('web page instead of JSON')) {
        return {
            toastMessage: 'Server returned HTML instead of JSON. If you use a reverse proxy, check its timeout settings (need at least 180s). Otherwise check the SillyTavern terminal for errors.',
            timeOut: 10000,
        };
    }
